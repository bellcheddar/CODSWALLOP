...
- [ ] **Artefacts for families a reader creates, not just ones somebody pre-warmed.** The embedding and the interaction fingerprint are built on a workstation and rsynced across, because the droplet has two cores shared with eight apps and deliberately has neither biotite, tmtools, PLIP nor OpenBabel installed. So a family assembled for the first time by a reader on the live site has an artefact on neither machine, and it renders the sequence-identity placeholder map: the page still draws, no error is raised anywhere, and the map quietly stops being a measurement of structural distance while continuing to look like one. `artefacts --missing` already reports the condition from the droplet, which is the one machine that cannot fix it, and nothing acts on that report. Needs a request queue the droplet appends to and the weekly warm drains, plus a visible marker on the map itself, because a silent downgrade to a different quantity is the part that matters and the pre-warmed set will never cover the whole PDB
...
- [ ] **Motifs tab.** Functional sites the family shares, in one place and in seed coordinates, so a motif is the same column as the conservation track, the domain ribbon, the density census and the PLIP hot residues, and binds to the shared selection state like every other panel. Sources, each attributed on the panel: **PROSITE** patterns and profiles via ScanProsite for catalytic signatures and PTM consensus sites; **ELM** for short linear motifs, which is what actually catches the docking and degron motifs PROSITE's curated patterns miss; **UniProt's own** `MOD_RES`, `LIPID`, `CARBOHYD`, `DISULFID` and `TRANSMEM` features, which the app already fetches and currently uses only to describe a mutation; **DSSP** for the secondary-structure spine, shared with the topology diagram above so the two cannot disagree; and **PDBTM** or **OPM** for membrane boundaries measured from the structure rather than predicted from the sequence. The family-centric part is the point: a predicted site that appears in one member is noise, and the same site conserved across 90 % of the family with a resolved side chain in forty entries is a finding, so every motif is reported with the fraction of the family carrying it and the fraction where it is actually resolved, never as a bare hit
- [ ] **Family-specific reference numbering in the motifs tab**, following what BoltzMaker does. **KLIFS** for kinases: the 85-residue pocket with its standardised numbering, so the DFG motif, the gatekeeper, the hinge and the αC-helix glutamate are named rather than counted, and the DFG-in/DFG-out and αC-in/αC-out states become a filter across the family. **GPCRdb** for receptors: Ballesteros-Weinstein generic numbering plus the DRY, NPxxY, CWxP and PIF micro-switches, and the receptor's activation state. Both give a family a vocabulary that its own residue numbers do not, which is exactly what a family-centric page should be speaking, and both make the cross-species orthologue matrix comparable in a way raw numbering cannot be. The tab must also say plainly when neither applies, rather than showing an empty kinase pocket for a lysozyme. Licences need checking before this ships: GPCRdb releases its data CC BY, KLIFS is free for academic use, and PROSITE's data carries a non-commercial clause, so the attribution block is a requirement here and not a courtesy
